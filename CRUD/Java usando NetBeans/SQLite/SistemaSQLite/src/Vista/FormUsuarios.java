...
public class FormUsuarios extends javax.swing.JFrame {
    ...
    public void cargarListaUsuarios() {
        ...
        ArrayList listaUsuarios = datos.listarUsuariosEnTabla(txtBuscar.getText());
    ...
    public void asignar(String tipo_usuario) {
        ...
        if (row >= 0) {
            AccesoDatos datos = new AccesoDatos();
            Usuario usuario = datos.listarUsuarioEnTabla(id_usuario_to_load);
            int id_usuario = usuario.getId_usuario();
    ...
    public void grabar() {
        ...
        if (validar()) {
            ...
            if (nuevo) {
                sql = "insert into usuarios(usuario,clave,tipo,fecha_registro) values('" + usuario.getNombre() + "','" + usuario.getPassword() + "'," + usuario.getTipo() + ",'" + usuario.getFecha_registro() + "')";
            } else {
                sql = "update usuarios set tipo=" + tipo + " where usuario='" + usuario.getNombre() + "'";
            }

            if (nuevo) {
                if (datos.comprobar_existencia_usuario_crear(usuario.getNombre())) {
                    JOptionPane.showMessageDialog(this, "El usuario " + usuario.getNombre() + " ya existe");
                    status.setText("[-] El usuario " + usuario.getNombre() + " ya existe");
                } else {
                    if (datos.cargarConsulta(sql)) {
