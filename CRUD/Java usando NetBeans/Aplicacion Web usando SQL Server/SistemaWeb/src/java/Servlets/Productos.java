                    ...
                    if (request.getParameter("agregar_producto") != null) {
                        ...
                        if (!"".equals(nombre_producto) && !"".equals(descripcion) && funcion.valid_number(precio) && funcion.valid_number(id_proveedor) && !"".equals(fecha_registro)) {
                            if (conexion_now.comprobar_existencia_producto_crear(nombre_producto)) {
                                out.println(funcion.Redirect("El producto " + nombre_producto + " ya existe", "administracion.jsp?productos"));
                            } else {
                                String sql = "insert into productos values('" + nombre_producto + "','" + descripcion + "'," + precio + "," + id_proveedor + ",'" + fecha_registro + "')";
                                if (conexion_now.cargarConsulta(sql)) {
                                    out.println(funcion.Redirect("Producto registrado", "administracion.jsp?productos"));
                                } else {
                                    out.println(funcion.Redirect("Ha ocurrido un error en la base de datos", "administracion.jsp?productos"));
                                }
                            }
                        } else {
                            out.println(funcion.Redirect("Faltan datos", "administracion.jsp?productos"));
                        }

                    } else if (request.getParameter("actualizar_producto") != null) {
                        ...
                        if (funcion.valid_number(id_producto) && !"".equals(nombre_producto) && !"".equals(descripcion) && funcion.valid_number(precio) && funcion.valid_number(id_proveedor)) {
                            if (conexion_now.comprobar_existencia_producto_editar(Integer.parseInt(id_producto), nombre_producto)) {
                                ...
                                if (conexion_now.cargarConsulta(sql)) {
                                    out.println(funcion.Redirect("Producto editado", "administracion.jsp?productos"));
                                } else {
                                    out.println(funcion.Redirect("Ha ocurrido un error en la base de datos", "administracion.jsp?productos"));
                                }
                            }
                        ...
                        if (funcion.valid_number(id_producto)) {
                            ...
                            if (conexion_now.cargarConsulta(sql)) {
                                out.println(funcion.Redirect("Producto borrado", "administracion.jsp?productos"));
                            } else {
                                out.println(funcion.Redirect("Ha ocurrido un error en la base de datos", "administracion.jsp?productos"));
                            }
                        } else {
                            out.println(funcion.Redirect("ID invalido", "administracion.jsp?productos"));
                        }
                    } else {
