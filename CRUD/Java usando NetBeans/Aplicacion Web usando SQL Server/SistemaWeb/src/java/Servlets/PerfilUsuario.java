                    ...
                    if (request.getParameter("cambiar_user") != null) {
                        ...
                        if (!"".equals(username) && !"".equals(password) && !"".equals(new_username)) {
                            ...
                            if (conexion_now.ingreso_usuario(username, password_encoded)) {
                                if (conexion_now.comprobar_existencia_usuario_editar(new_username)) {
                                    out.println(funcion.Redirect("El usuario " + new_username + " ya existe", "administracion.jsp?cambiar_usuario"));
                                } else {
                                    String sql = "update usuarios set usuario='" + new_username + "' where usuario='" + username + "'";
                                    if (conexion_now.cargarConsulta(sql)) {
                                        out.println(funcion.Redirect("Nombre de usuario cambiado , se debe reiniciar el sistema", "Login?LogOut"));
                                    } else {
                                        out.println(funcion.Redirect("Ha ocurrido un error en la base de datos", "administracion.jsp?cambiar_usuario"));
                                    }
                                }
                            } else {
                                out.println(funcion.Redirect("Login incorrecto", "administracion.jsp?cambiar_usuario"));
                            }
                        } else {
                            out.println(funcion.Redirect("Faltan datos", "administracion.jsp?cambiar_usuario"));
                        }

                    } else if (request.getParameter("cambiar_pass") != null) {

                        String username = request.getParameter("username");
                        String password = request.getParameter("anterior_password");
                        ...
                        if (!"".equals(username) && !"".equals(password) && !"".equals(new_password)) {
                            ...
                            if (conexion_now.ingreso_usuario(username, password_encoded)) {
                                ...
                                if (conexion_now.cargarConsulta(sql)) {
                                    out.println(funcion.Redirect("Password de usuario cambiado , se debe reiniciar el sistema", "Login?LogOut"));
                                } else {
                                    out.println(funcion.Redirect("Ha ocurrido un error en la base de datos", "administracion.jsp?cambiar_password"));
                                }
                            } else {
                                out.println(funcion.Redirect("Login incorrecto", "administracion.jsp?cambiar_password"));
                            }
                        } else {
                            out.println(funcion.Redirect("Faltan datos", "administracion.jsp?cambiar_password"));
                        }
