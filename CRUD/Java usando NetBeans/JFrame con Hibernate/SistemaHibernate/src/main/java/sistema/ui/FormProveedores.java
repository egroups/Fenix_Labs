...
public class FormProveedores extends javax.swing.JFrame {
    ...
    public void cargarListaProveedores() {
        ...
        ArrayList listaProveedores = datos.listarProveedoresEnTabla(txtBuscar.getText());

        for (int i = 0; i < listaProveedores.size(); i++) {
            Proveedores proveedor = (Proveedores) listaProveedores.get(i);
            int id_proveedor = proveedor.getIdProveedor();
            ...
            String fecha_registro = funcion.convertir_fecha_para_usuario(proveedor.getFechaRegistroProveedor());
    ...
    public void cargarCamposProveedor(int id_proveedor_to_load) {
        ...
        AccesoDatos datos = new AccesoDatos();
        Proveedores proveedor = datos.cargarProveedorEnTabla(id_proveedor_to_load);
        int id_proveedor = proveedor.getIdProveedor();
        String nombre_empresa = proveedor.getNombreEmpresa();
        String direccion = proveedor.getDireccion();
        int telefono = proveedor.getTelefono();
    ...
    public void borrar() {
        ...
        if (row >= 0) {
            ...
            if (resultado == JOptionPane.YES_OPTION) {
                ...
                if (datos.borrarProveedor(id_proveedor)) {
    ...
    public void grabar() {
        ...
        if (validar()) {
            ...
            p.setNombreEmpresa(txtNombre.getText());
            p.setDireccion(txtDireccion.getText());
            p.setTelefono(Integer.parseInt(txtTelefono.getText()));
            p.setFechaRegistroProveedor(funcion.fecha_del_dia());

            if (nuevo) {
                if (datos.comprobar_existencia_proveedor_crear(p.getNombreEmpresa())) {
                    JOptionPane.showMessageDialog(this, "El proveedor " + p.getNombreEmpresa() + " ya existe");
                    status.setText("[-] El proveedor " + p.getNombreEmpresa() + " ya existe");
                } else {
                    if (datos.agregarProveedor(p.getNombreEmpresa(), p.getDireccion(), p.getTelefono(), p.getFechaRegistroProveedor())) {
                ...
                if (datos.comprobar_existencia_proveedor_editar(p.getIdProveedor(), p.getNombreEmpresa())) {
                    JOptionPane.showMessageDialog(this, "El proveedor " + p.getNombreEmpresa() + " ya existe");
                    status.setText("[-] El proveedor " + p.getNombreEmpresa() + " ya existe");
                } else {
                    if (datos.editarProveedor(p.getIdProveedor(), p.getNombreEmpresa(), p.getDireccion(), p.getTelefono(), p.getFechaRegistroProveedor())) {
