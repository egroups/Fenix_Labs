...
public class FormProductos extends javax.swing.JFrame {
    ...
    public void cargarListaProductos() {
        ...
        ArrayList listaProductos = datos.listarProductosEnTabla(txtBuscar.getText());
        ...
        for (int i = 0; i < listaProductos.size(); i++) {
            ...
            String nombre_producto = producto.getNombreProducto();
            String descripcion = producto.getDescripcion();
            double precio = producto.getPrecio();
            String fecha_registro = funcion.convertir_fecha_para_usuario(producto.getFechaRegistro());
            int id_proveedor = producto.getProveedores().getIdProveedor();

            String nombre_proveedor = datos.cargarNombreProveedor(id_proveedor);
    ...
    public void cargarComboBoxProveedores() {
        ...
        ArrayList proveedores = datos.listarProveedoresEnTabla(txtBuscar.getText());

        DefaultComboBoxModel modelo1 = new DefaultComboBoxModel();

        for (int i = 0; i < proveedores.size(); i++) {
            Proveedores proveedor = (Proveedores) proveedores.get(i);
            String nombre_empresa = proveedor.getNombreEmpresa();
    ...
    public void cargarCamposProducto(int id_producto_to_load) {
        ...
        AccesoDatos datos = new AccesoDatos();
        Productos producto = datos.cargarProductoEnTabla(id_producto_to_load);
    ...
    public void borrar() {
        ...
        if (row >= 0) {
            ...
            if (resultado == JOptionPane.YES_OPTION) {
                if (datos.borrarProducto(id_producto)) {
    ...
    public void grabar() {
        ...
        if (validar()) {
            ...
            Double d = new Double(Double.parseDouble(txtPrecio.getText()));
            p.setPrecio(d.intValue());
            p.setFechaRegistro(funcion.fecha_del_dia());

            int codigoProveedor = ((Proveedores) lista_proveedores.get(cmbProveedor.getSelectedIndex())).getIdProveedor();

            if (nuevo) {
                if (datos.comprobar_existencia_producto_crear(p.getNombreProducto())) {
                    JOptionPane.showMessageDialog(this, "El producto " + p.getNombreProducto() + " ya existe");
                    status.setText("[-] El producto " + p.getNombreProducto() + " ya existe");
                } else {
                    if (datos.agregarProducto(p.getNombreProducto(), p.getDescripcion(), p.getPrecio(), codigoProveedor, p.getFechaRegistro())) {
                ...
                if (datos.comprobar_existencia_producto_editar(p.getIdProducto(), p.getNombreProducto())) {
                    JOptionPane.showMessageDialog(this, "El producto " + p.getNombreProducto() + " ya existe");
                    status.setText("[-] El producto " + p.getNombreProducto() + " ya existe");
                } else {
                    if (datos.editarProducto(p.getIdProducto(), p.getNombreProducto(), p.getDescripcion(), p.getPrecio(), codigoProveedor, p.getFechaRegistro())) {
